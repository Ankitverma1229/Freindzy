...
import { MailIcon, LockIcon, EyeIcon, EyeOffIcon, User } from "lucide-react";
import {
  sendLoginDetails,
...
const AuthForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isRegister) {
      sendRegistrationDetails(formData, navigate);
      return;
    }
    sendLoginDetails(formData, navigate);
  };
  ...
  return (
    <div className="flex flex-col items-center w-full    rounded-lg ">
      <form onSubmit={handleSubmit} className="w-full space-y-5">
        ...
        <button
          type="submit"
          className="w-full bg-[#24AE7C] py-2 rounded-md text-white font-semibold hover:bg-[#1E9B6B] transition"
        >
